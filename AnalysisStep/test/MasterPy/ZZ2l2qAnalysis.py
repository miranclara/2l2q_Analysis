...
declareDefault("SELSETUP", "allCutsAtOnce", globals())#changed

#Best candidate comparator (see interface/Comparators.h)
#declareDefault("BESTCANDCOMPARATOR", "byBestKD", globals())
declareDefault("BESTCANDCOMPARATOR", "byBestZqq", globals())#changed
...
if SELSETUP=="Legacy" and not BESTCANDCOMPARATOR=="byBestZqq":#changed
    print "WARNING: In ZZ2l2qAnalysis.py the SELSETUP=\"Legacy\" flag is meant to reproduce the Legacy results, ignoring the setting of the BESTCANDCOMPARATOR: ",BESTCANDCOMPARATOR
...
ELEISOCUT = 0.35 # [FIXME] Remove isolation cuts from the code completely #changed
MUISOCUT  = 0.35 # [FIXME] Remove isolation cuts from the code completely,#changed
...
if (SAMPLE_TYPE == 2016):
    ...
    if IsMC:
        process.GlobalTag = GlobalTag(process.GlobalTag, '102X_upgrade2018_realistic_v21', '')#changed
    else:
        if (DATA_TAG == "PromptReco"):
            process.GlobalTag = GlobalTag(process.GlobalTag, '102X_dataRun2_Prompt_v16', '')#diff
        else:
            process.GlobalTag = GlobalTag(process.GlobalTag, '102X_dataRun2_v13', '')#diff
...
# Filter for Zbb enrichment:added
...
SIP =  "userFloat('SIP') < 4" #diff
#GOODMUON = "(userFloat('ID') || (userFloat('isTrackerHighPtMuon') && pt>200)) && " + SIP //used when MVA is applied
GOODELECTRON = "userFloat('ID') && " + SIP #diff
GOODMUON     = "userFloat('ID') && " + SIP #diff
...
#                                   passthrough = cms.string("isGlobalMuon && numberOfMatches >= 2"),
                                   passthrough = cms.string("(isGlobalMuon && numberOfMatches >= 2)||(pt>=0)"),#diff, turn on ghosto cleaning added(pt>=0);
...
Z2PRESEL    = ("mass > 40 && mass < 150") # Note: this is without ISO :added
BESTZ2_AMONG = ( Z2PRESEL ) ## && userFloat('d1.passCombRelIsoPFFSRCorr')" ):added
...
process.bareZjjCand = cms.EDProducer("CandViewShallowCloneCombiner",#changed
...
#    cut = cms.string('deltaR(daughter(0).eta, daughter(0).phi, daughter(1).eta, daughter(1).phi)>0.02 && abs(daughter(0).pdgId())==abs(daughter(1).pdgId())'), # protect against ghosts && same flavour
    cut = cms.string('pt>100 && mass>40 && mass <180'), # protect against ghosts && same flavour 
    checkCharge = cms.bool(False)
)
#process.LLCand = cms.EDProducer("ZCandidateFiller",
process.ZjjCand = cms.EDProducer("ZCandidateFiller",#changed
    src = cms.InputTag("bareZjjCand"),#changed
    sampleType = cms.int32(SAMPLE_TYPE),
    embedDaughterFloats = cms.untracked.bool(True),#added
    setup = cms.int32(LEPTON_SETUP), # define the set of effective areas, rho corrections, etc.
#    bestZAmong = cms.string(BESTZ_AMONG),
    bestZAmong = cms.string(BESTZ2_AMONG),#changed
#    FSRMode = cms.string(FSRMODE), # "skip", "Legacy", "RunII"
    FSRMode = cms.string("skip"), # "skip", "Legacy", "RunII",changed
    flags = cms.PSet(
#        GoodLeptons = cms.string(ZLEPTONSEL),
        GoodLeptons = cms.string('0'),
        Z1Presel = cms.string(Z2PRESEL),#Z1PRESEL->Z2PRESEL,clara
    )
)


### ----------------------------------------------------------------------
### TriLeptons (for fake rate):changed=disactivated
### ----------------------------------------------------------------------
...
#                       "&& userFloat('d0.worstMuIso') <" + str(MUISOCUT) +#changed
                       "&& userFloat('d1.worstMuIso') <" + str(MUISOCUT)
                       ) #ZZ made of 4 tight leptons passing SIP and ISO


#Z1MASS            = "daughter('Z1').mass>40 && daughter('Z1').mass<120" #changed
#Z2MASS            = "daughter('Z2').mass>4  && daughter('Z2').mass<120" # (was > 4 in Synch) to deal with m12 cut at gen level#changed
Z1MASS            = "daughter('Z1').mass>40 && daughter('Z1').mass<180 && daughter('Z1').pt>100"#changed
JETMASS           = "daughter('Z1').pt>170. && abs(daughter('Z1').eta)<2.4" #added
Z2MASS            = "daughter('Z2').mass>55 && daughter('Z2').mass<120 && daughter('Z2').pt>100" # changed (was > 4 in Synch) to deal with m12 cut at gen level
...
#disactivated if SELSETUP=="Legacy to elif SELSETUP=="allCutsAtOnce"
...
print "using the cut ",SELSETUP #added
if SELSETUP=="allCutsAtOnce": # Select best candidate among those passing all cuts
    print "using allCutsAtOnce"
...
#                      MLLALLCOMB      + "&&" + #changed
                      ...
                      "mass>300"      
#                      "mass>70"       + "&&" +#changed
#                      "daughter('Z2').mass>12" #changed
                      )

    BESTCAND_AMONG2 = (FOURGOODLEPTONS + "&&" + #? Clara
                      JETMASS          + "&&" +
                      Z2MASS          + "&&" +
#                      MLLALLCOMB      + "&&" +#changed
                      PT20_10         + "&&" +
...
#                      "daughter('Z2').mass>12" #changed
    ...
    SR = BESTCAND_AMONG
    SR2 = BESTCAND_AMONG2 #added


#commented from elif SELSETUP=="allCutsAtOnceButMZ2": to elif SELSETUP=="allCutsAtOncePlusSmart":
...
NOGHOST4l = ("deltaR(daughter(0).daughter(0).eta, daughter(0).daughter(0).phi, daughter(1).daughter(0).eta, daughter(1).daughter(0).phi)>0.02 &&"+
             ...
             "abs(daughter(0).daughter(0).eta)<2.4 && "+#differ
             "abs(daughter(0).daughter(1).eta)<2.4") # jets only in tracker, differ
...
# ZZ Candidates
if FSRMODE == "Legacy":
    RECOMPUTEISOFORFSR = True#added
    print "\nERROR: FSRMODE=Legacy is no longer supported. Aborting...\n"
    exit(1)
else :#added
    RECOMPUTEISOFORFSR = False#added



#process.bareZZCand= cms.EDProducer("CandViewShallowCloneCombiner",#differ:"PATCandViewShallowCloneCombiner",
process.bareZZCand= cms.EDProducer("PATCandViewShallowCloneCombiner",#differ:"PATCandViewShallowCloneCombiner",
    decay = cms.string('ZjjCand ZCand'),
#    decay = cms.string('ZCand ZCand'),
    cut = cms.string(LLLLPRESEL),
#    checkCharge = cms.bool(True) #changed, 4L
    checkCharge = cms.bool(False) #changed,2l2q
)
process.ZZCand = cms.EDProducer("ZZjjCandidateFiller",#changed
#process.ZZCand = cms.EDProducer("ZZCandidateFiller",
    ...
    recomputeIsoForFSR = cms.bool(RECOMPUTEISOFORFSR),#added
...
#process.bareZLLCand= cms.EDProducer("CandViewShallowCloneCombiner",#changed
#    decay = cms.string('ZCand LLCand'),#changed
#    cut = cms.string(NOGHOST4l),#changed
...
#    checkCharge = cms.bool(False)#changed
#)#changed
#commented process.ZLLCand
...
from RecoJets.JetProducers.PileupJetID_cfi import _chsalgos_94x, _chsalgos_102x #changed,diff
from RecoJets.JetProducers.PileupJetIDCutParams_cfi import full_81x_chs_wp #changed,diff
from PhysicsTools.PatAlgos.tools.jetTools import updateJetCollection #added, diff
...
if (LEPTON_SETUP == 2016): #DeepCSV, from https://twiki.cern.ch/twiki/bin/viewauth/CMS/BtagRecommendation2016Legacy
   ...
   theBTagMCEffFile="ZZAnalysis/AnalysisStep/data/BTagging/bTagEfficiencies_2016_LegacyPaper.root" #changed
   ...
   theBTagMCEffFile="ZZAnalysis/AnalysisStep/data/BTagging/bTagEfficiencies_2017_LegacyPaper.root" #changed
   ...
   theBTagMCEffFile="ZZAnalysis/AnalysisStep/data/BTagging/bTagEfficiencies_2018_LegacyPaper.root" #change
else:
...
process.dressedJets = cms.EDProducer("JetFiller",
    ...
    sampleType = cms.int32(SAMPLE_TYPE),
    setup = cms.int32(LEPTON_SETUP),
    cut = cms.string("pt>20 && abs(eta)<4.7 && userFloat('JetID') && (userFloat('PUjetID') || pt>50)"),#corret one
#    cut = cms.string("pt>30 && abs(eta)<4.7 && userFloat('JetID') && userFloat('PUjetID')"),#changed
    isMC = cms.bool(IsMC),
...
process.cleanJets = cms.EDProducer("JetsWithLeptonsRemover",
                                   ...
                                   ElectronPreselection = cms.string("userFloat('isGood') && userFloat('passCombRelIsoPFFSRCorr')"), #diff
                                  # ElectronPreselection = cms.string("userFloat('isGood')"), #diff
...
# Build 4-lepton candidates
       ...
       process.bareZjjCand       + process.ZjjCand     + #added
